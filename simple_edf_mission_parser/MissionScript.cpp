...
void CMissionScript::Write( const std::wstring& path, int flags )
{
	...
	int InitFnsize = 0; //now it's useless

	for (int i = 0; i < m_Initialisers.size(); i++)
	{
		m_Initialisers[i]->CompileInit();

		InitBytes.insert(InitBytes.end(), m_Initialisers[i]->bytes.begin(), m_Initialisers[i]->bytes.end());
	...
	for (int i = 0; i < m_Initialisers.size(); i++)
	{
		PushWStringToVector(m_Initialisers[i]->initName, &bytes);
	}
...
void MissionFunction::CompileInitValue(std::wstring argsStrn)
{
	...
	else if (argsStrn.back() == L'h')
	{
		//erase Identifier
		argsStrn.erase(argsStrn.size()-1, 1);
