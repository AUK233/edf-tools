...
PS_OUTPUT main(PS_INPUT input)
{
    ...
    float3 albedo_sample = albedo.Sample(albedo_sampler, input.texcoord).xyz;
    ...
    float3 s_normal = normal.Sample(normal_sampler, input.texcoord).xyz;
	...
	// Damage-related calculations
	float2 damageUV = input.texcoord * damage_uv_mul;  // Scale UV for damage texture
	float2 damageMaskUV = input.texcoord * damage_mask_uv_mul;  // Scale UV for damage mask texture

	// Sample the damage distance texture
	float damageDist = damage_dist.Sample(damage_dist_sampler, damageUV).r;

	// Sample the damage diffuse texture
	float3 damageDiffuse = damage_diffuse.Sample(damage_diffuse_sampler, damageUV).rgb;

	// Sample the damage normal texture
	float3 damageNormalSample = damage_normal.Sample(damage_normal_sampler, damageUV).rgb;
	float3 damageNormal = CalculateNormal(damageNormalSample, normalize(input.normal), normalize(input.binormal), normalize(input.tangent));

	// Calculate the damage amount based on the damage distance and threshold
	float damageAmount = saturate(damageDist - damage_test);  // Clamp damage amount between 0 and 1

	// Blend the diffuse color with the damage color
	final_diffuse = lerp(final_diffuse, damageDiffuse, damageAmount);

	// Blend the normal with the damage normal
	final_normal = lerp(final_normal, damageNormal, damageAmount * damage_normal_amount);

	// Adjust the roughness and metallic based on damage
	float final_Roughness = lerp(roughness, damage_roughness, damageAmount);
	float final_Metallic = lerp(metallic, damage_metallic, damageAmount);

	// Adjust the specular color based on damage
	final_specular = lerp(final_specular, damage_specular_color, damageAmount);

	// Adjust the edge color for damaged areas
	float3 edgeColor = lerp(damage_edge_color, damage_max_color, damageAmount);

	// Apply the damage light mask
	float lightMask = lerp(1.0, damage_light_mask, damageAmount);
	...
	// other
	output.o3 = 1;
    output.o4 = float4(final_Roughness, final_Metallic, 0.0, 1.0);
