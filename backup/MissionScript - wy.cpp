...
std::vector< char > BVMHeader::GenerateBytes()
{
	...
	//Pointer to initialization values, 0x30
	//This is necessary, it is unable to 0
	...
	//Pointer to name of initialize the global variable string, only one string
	//Use this to check the location, 0x3C
	//std::wcout << std::hex << int(bytes.size());
	for (int i = 0; i < 4; i++)
	{
		bytes.push_back(0x00);
	}
	...
	//Pointer to end of 4-byte aligned valid data, 0x44
	seg = IntToBytes( 0 );
	for( int i = 0; i < 4; i++ )
	{
		bytes.push_back( seg[i] );
	...
	//pointer to EOF, 0x4C
	//output location for later modification
	EOFpos.push_back(bytes.size());
	for( int i = 0; i < 4; i++ )
	{
		bytes.push_back(0x00);
	}
...
void CMissionScript::Write( const std::wstring& path, int flags )
{
	...
	for( int i = 0; i < missionSourceProccessed.size( ); i++ )
	{
		...
		if( missionSourceProccessed[i] == L' ' )
		{
			...
			if (varBuf == L"initialize")
			{
				...
				m_Initialisers.push_back(new MissionFunction(fnBuffer, this));
	...
	for( int i = 0; i < m_vecFunctions.size( ); i++ )
	{
		...
		/*
		if (m_vecFunctions[i]->fnRetType == 255)
		{
			hasInitFn = true;
			m_vecFunctions[i]->Compile();

			InitBytes.insert(InitBytes.end(), m_vecFunctions[i]->bytes.begin(), m_vecFunctions[i]->bytes.end());
			header->sizeOfVarInitialisers = InitBytes.size();

			InitFnsize = 1;
			std::wcout << int(InitBytes.size());
		}*/
	}

	//Parse and generate initialiser bytes
	std::vector< char > InitBytes;
	bool hasInitFn = false;
	int InitFnsize = 0;

	for (int i = 0; i < m_Initialisers.size(); i++)
	{
		m_Initialisers[i]->CompileInit();
	...
	if (!hasInitFn)
	{
		...
		header->sizeOfVarInitialisers = 1;
	}
	...
	header->fnArrSize = m_vecFunctions.size( ) - InitFnsize;
	header->fnArrPtr = header->varArrPtr + (header->varArrSize * 4);

	int fnDataSize = header->fnArrSize * 16;
	int startOfFnData = header->fnArrPtr + fnDataSize + header->sizeOfVarInitialisers;
	...
	for( int i = 0; i < m_vecFunctions.size( ); i++ )
	{
		if (m_vecFunctions[i]->fnRetType == 0)
		{
			m_vecFunctions[i]->Compile();
	...
	for( int i = 0; i < m_vecFunctions.size( ); i++ )
	{
		if (m_vecFunctions[i]->fnRetType == 0)
		{
			for (int j = 0; j < m_vecFunctions[i]->patchBytePos.size(); j++)
			{
				...
				if (fnOfs > 32767)
				{
					std::wcout << "\noverly large location:" + fnOfs;
					//It doesn't make sense to support it now
					/*
					Fnbytes[start + 3] = seg[2];
					Fnbytes[start + 4] = seg[3];
					Fnbytes[start + 0] = 0xE9;
					*/
				}
	...
	int startOfStrings = header->fnArrPtr + fnDataSize + header->sizeOfVarInitialisers + Fnbytes.size( ) + fnPointerDataSize;
	...
	offset = 0;
	int strOfs = 0;
	//If there is no related content initialized in the text
	//An empty content will be output
	for( int i = 0; i < m_vecFunctions.size( ); i++ )
	{
		if (m_vecFunctions[i]->fnRetType == 0)
		{
			std::vector< char > dataBytes = m_vecFunctions[i]->GenerateFnDataBytes(startOfFunctionStrings + strOfs, startOfStrings - fnPointerDataSize, i - InitFnsize, offset);
	...
	int fnDataBytesOffset = 0;
	for( int i = 0; i < fnDataBytes.size( ); i++ )
	{
		bytes.push_back( fnDataBytes[i] );
		if (fnDataBytesOffset == 0)
		{
			fnDataBytesOffset = bytes.size();
		}
	}
	...
	for (int i = 0; i < m_vecFunctions.size(); i++)
	{
		int argsnum = m_vecFunctions[i]->fnArgBytes.size();
		if (argsnum > 0)
		{
			for (int j = 0; j < argsnum; j++)
			{
				bytes.push_back(m_vecFunctions[i]->fnArgBytes[j]);
			}

			// Override offset address of fnArgs
			// +7 here because the position obtained is already 1 larger than it should be
			int start = (i - InitFnsize) * 16 + fnDataBytesOffset + 7;
			// only debug
			//std::wcout << int(start);
			//std::wcout << "\n";
			char* seg = IntToBytes((bytes.size() - argsnum));
			bytes[start + 0] = seg[0];
			bytes[start + 1] = seg[1];
			bytes[start + 2] = seg[2];
			bytes[start + 3] = seg[3];
	...
	//Push init String
	int initpos = header->EOFpos.back() - 16;
	char* initseg = IntToBytes(bytes.size());
	bytes[initpos + 0] = initseg[0];
	bytes[initpos + 1] = initseg[1];
	bytes[initpos + 2] = initseg[2];
	bytes[initpos + 3] = initseg[3];
	free(initseg);
	...
	//If it is not present, output a default name
	if (hasInitFn == false)
	{
		PushWStringToVector(L"Mission", &bytes);
		std::wcout << L"No global variables are initialized, the output defaults.\n";
	}

	//Functions

	//Check the alignment
	bool eoaPosSet = false;
	int eod = header->EOFpos.back() - 8;
	int filesize = bytes.size() % 16;
	//If the tail is less than 16 bytes, it is completely filled.
	if (filesize > 0)
	{
		for (int i = filesize; i < 16; i++)
		{
			bytes.push_back(0xBA);
			if (bytes.size() % 4 == 0 && eoaPosSet == false)
			{
				char* eofseg = IntToBytes(bytes.size());
				bytes[eod + 0] = eofseg[0];
				bytes[eod + 1] = eofseg[1];
				bytes[eod + 2] = eofseg[2];
				bytes[eod + 3] = eofseg[3];
				free(eofseg);

				eoaPosSet = true;
			}
		}
		//std::fill_n(std::back_inserter(bytes), 16 - filesize, 0xBA);
	}

	//write EOF
	int eof = header->EOFpos.back();
	char* eofseg = IntToBytes(bytes.size());
	bytes[eof + 0] = eofseg[0];
	bytes[eof + 1] = eofseg[1];
	bytes[eof + 2] = eofseg[2];
	bytes[eof + 3] = eofseg[3];
	if (eoaPosSet == false)
	{
		bytes[eod + 0] = eofseg[0];
		bytes[eod + 1] = eofseg[1];
	...
	std::wcout << L"Compilation completed: " + sourcePath + L"\n";
...
void MissionFunction::CompileLine(std::wstring fnStrn)
{
	...
	else if (preBracketString == L"}") //Patch conditional
	{
		...
		if (patchConditionalPos.size() > 0)
		{
			...
			//If there is an else position
			//Check if the position is an always jump
			//Its hex should be: E8 0500 0000, 0xE8 = -24 && int(bytes[start - 4]) == 5 && int(bytes[start - 3]) == 0 :useless
			//Its hex should be: A8 0300
			if (elseConditionalPos.size() > 0 && int(bytes[start - 3]) == -88 && int(bytes[start - 2]) == 3 && int(bytes[start - 1]) == 0)
			{
				char* seg = IntToBytes((bytes.size() - start) + 3);
			...
			{
				//Always jump to the next position, used to implement else
				bytes.push_back(0xA8);
				bytes.push_back(0x03);
				bytes.push_back(0x00);
				//Check if the position is a jump
				//Its hex should be: E6 0000 0000, 0xE6 = -26:useless
				//Its hex should be: A6 0000
				if (int(bytes[start - 3]) == -90 && int(bytes[start - 2]) == 0 && int(bytes[start - 1]) == 0)
				{
					char* seg = IntToBytes((bytes.size() - start) + 3);
...
void MissionFunction::CompileValue( std::wstring argsStrn )
{
	...
	else //user *probably* wants an int
	{
		...
		{
			//use 1-byte versions of 0 and 1
			if (num == 0)
			{
...
void MissionFunction::Compile( )
{
	...
	else if (parser == L"initialize")
	{
		// Provides functionality to set default values for global variables.
		// wait for completion.
		isInit = true;
	}
	...
	{
		//Verbose
		std::wcout << L"Compiling function: " + fnName;
		...
		while (token2)
		{
			...
			if (argParser == L"int")
			{
				...
				if (token2)
				{
					...
					fnArgBytes.push_back(0x01);
				}
			}
			else if (argParser == L"float")
			{
				token2 = wcstok(NULL, L"(), ");
				if (token2)
				{
			...
			else if (argParser == L"string")
			{
				...
				if (token2)
				{
					...
					m_iNumLocalVars2++;
					m_vecLocalVars.push_back(token2);
					fnArgBytes.push_back(0x03);
				}
			}
			token2 = wcstok(NULL, L"(), ");
		}

		if (fnArgBytes.size() != NULL)
		{
			std::wcout << " has ";
			std::wcout << m_iNumLocalVars2;
			std::wcout << " parameter(s)";
		}
		std::wcout << L"\n";
	...
	{
		std::wcout << L"Initialize Global Variables:\n" + fnName;
...
void MissionFunction::CompileInit()
{
	std::wstring source = sourceCode;
	...
	{
		//Attempt rewrite code
		CompileInitLine(lines[i]);
	}

	std::wcout << " has ";
	std::wcout << m_iNumGlobalVars;
	std::wcout << " value";
	std::wcout << L"\n";
...
void MissionFunction::CompileInitLine(std::wstring fnStrn)
{
	std::wstring argsStrn;
	bool parsingArgStrn = false;
	int bracketDepth = 0;
	...
	for (int i = 0; i < myScript->GetNumVars(); i++)
	{
		if (preBracketString == myScript->GetVarName(i))
		{
			while (argsStrn.size() > 0)
			{
				std::wstring arg;
...
void MissionFunction::CompileInitValue(std::wstring argsStrn)
{
	// only support int, float and hex
	if (argsStrn.back() == L'f')
	{
		float num = stof(argsStrn);
		bytes.push_back(0xD5);
		char floatBytes[4] = { 0x0,0x0,0x0,0x0 };
...
std::vector< char > MissionFunction::GenerateFnDataBytes( int ofsToName, int fnPointerSize, int fnNum, int offset )
{
	...
	if (m_iNumLocalVars2)
	{
		char* segFn = IntToBytes(m_iNumLocalVars2);
		bytes.push_back(segFn[0]);
		bytes.push_back(segFn[1]);
		bytes.push_back(segFn[2]);
		bytes.push_back(segFn[3]);
		free(segFn);
	}
	else
	{
