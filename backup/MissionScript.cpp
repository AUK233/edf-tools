...
void MissionFunction::CompileLine(std::wstring fnStrn)
{
	...
	else if (preBracketString == L"}") //Patch conditional
	{
		...
		if (patchConditionalPos.size() > 0)
		{
			...
			//Its hex should be: E8 0500 0000
			if (elseConditionalPos.size() > 0 && int(bytes[start - 5]) == -24 && int(bytes[start - 4]) == 5 && int(bytes[start - 3]) == 0 && int(bytes[start - 2]) == 0 && int(bytes[start - 1]) == 0)
			{
			...
			else
			{
				...
				//Its hex should be: E6 0000 0000
				if (int(bytes[start - 5]) == -26 && int(bytes[start - 4]) == 0 && int(bytes[start - 3]) == 0 && int(bytes[start - 2]) == 0 && int(bytes[start - 1]) == 0 && int(bytes[start - 2]) == 0 && int(bytes[start - 1]) == 0)
				{
					char* seg = IntToBytes((bytes.size() - start) + 5);
